...
public class SWFToolsSWFConverter implements SWFConverter {
	...
	public void convert2SWF(String inputFile, String swfFile, String extend) {
		...
		if (!pdfFile.exists()) {
			 com.bjpowernode.core.util.LogUtil.info("PDF文件不存在！");
			return;
		}
	
		if (outFile.exists()) {
			 com.bjpowernode.core.util.LogUtil.info("SWF文件已存在！");
			return;
		}
...
//		---update-for--文件预览无效----- scott@2013/05/22 ADD END
		try {
			...
			try {
				process.waitFor();
				com.bjpowernode.core.util.LogUtil.info("时间-------"+process.waitFor());

...
function dataFlush(){
	$.ajax({
        ...
        menujson = data;
        menujson = menujson.substring(0,menujson.indexOf("\"jsonStr\":")+1);
        menujson = menujson.substring(0,menujson.length-3);
//          		menujson = menujson.replace("{\"msg\":\"","");
  		menujson = menujson + "\"}";
  		var menu = eval("(" +menujson+")");
...
Navbar =function(me){
	...
	dataFlush();
	var _box = "<div  id='navbar' class='no_sysbtn' style='width:auto;'></div>";
	...
	var _abox  = "<a class='indicator indicator_{id}' href='###' cmd='switch' index='{index}'	title='{name}'><span class='indicator_icon_bg'></span><span class='indicator_icon_{id}' style='color:#FFFFFF;'>{name}</span></a>";
	var _abox2 = "<a class='indicator indicator_{key}' href='###' cmd='{key}' title='{title}'></a>";
	var num =totalnum ,////替换掉一级菜单的总数量
		defaultnum=1,
		title = "请登录",
